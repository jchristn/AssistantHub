    ...
    public class IngestionRuleMethods : IIngestionRuleMethods
    {
        ...
        public async Task<IngestionRule> CreateAsync(IngestionRule rule, CancellationToken token = default)
        {
            ...
            string query =
                ...
                "(id, tenant_id, name, description, bucket, collection_name, collection_id, verbex_index_id, labels_json, tags_json, atomization_json, summarization_json, chunking_json, embedding_json, created_utc, last_update_utc) " +
                "VALUES (" +
        ...
        public async Task<EnumerationResult<IngestionRule>> EnumerateAsync(string tenantId, EnumerationQuery query, CancellationToken token = default)
        {
            ...
            string countQuery = "SELECT COUNT(*) AS cnt FROM ingestion_rules WHERE tenant_id = '" + _Driver.Sanitize(tenantId) + "'";
