...
namespace AssistantHub.Core.Database.Postgresql.Implementations
{
    ...
    /// <summary>
    /// PostgreSQL crawl operation methods implementation.
    /// </summary>
    public class CrawlOperationMethods : ICrawlOperationMethods
    {
        ...
        public CrawlOperationMethods(PostgresqlDatabaseDriver driver, DatabaseSettings settings, LoggingModule logging)
        {
        ...
        public async Task<EnumerationResult<CrawlOperation>> EnumerateAsync(string tenantId, EnumerationQuery query, CancellationToken token = default)
        {
            ...
            string countQuery =
                "SELECT COUNT(*) AS cnt FROM crawl_operations " + whereClause + ";";
            ...
            if (selectResult != null && selectResult.Rows.Count > 0)
            {
                ...
                {
                    ret.Objects.Add(CrawlOperation.FromDataRow(row));
                }
            }
