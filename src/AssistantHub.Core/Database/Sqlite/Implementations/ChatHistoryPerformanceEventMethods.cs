...
namespace AssistantHub.Core.Database.Sqlite.Implementations
{
    ...
    using SyslogLogging;
        ...
        /// <summary>
        /// SQLite chat history performance event methods.
        /// </summary>
        public class ChatHistoryPerformanceEventMethods : IChatHistoryPerformanceEventMethods
    {
        ...
        /// <summary>
        /// Instantiate the SQLite performance-event data access layer.
        /// </summary>
        /// <param name="driver">SQLite database driver.</param>
        /// <param name="settings">Database settings.</param>
        /// <param name="logging">Logging module.</param>
        public ChatHistoryPerformanceEventMethods(SqliteDatabaseDriver driver, DatabaseSettings settings, LoggingModule logging)
        {
        ...
        public async Task<List<ChatHistoryPerformanceEvent>> ListByChatHistoryIdAsync(string chatHistoryId, CancellationToken token = default)
        {
            ...
            string query = "SELECT * FROM chat_history_performance_events WHERE chat_history_id = '" + _Driver.Sanitize(chatHistoryId) + "' ORDER BY sequence_number ASC, created_utc ASC;";
        ...
        {
            if (String.IsNullOrEmpty(chatHistoryId)) throw new ArgumentNullException(nameof(chatHistoryId));
            string query = "DELETE FROM chat_history_performance_events WHERE chat_history_id = '" + _Driver.Sanitize(chatHistoryId) + "';";
            await _Driver.ExecuteQueryAsync(query, true, token).ConfigureAwait(false);
        }
        ...
        public async Task DeleteExpiredAsync(int retentionDays, CancellationToken token = default)
        {
            ...
            string query = "DELETE FROM chat_history_performance_events WHERE created_utc < '" + _Driver.FormatDateTime(cutoff) + "';";
            await _Driver.ExecuteQueryAsync(query, true, token).ConfigureAwait(false);
        }
        ...
        private string BuildInsert(ChatHistoryPerformanceEvent evt)
        {
            ...
            return
                ...
                _Driver.FormatNullableString(evt.Phase) + ", " +
                _Driver.FormatNullableString(evt.Kind) + ", " +
                _Driver.FormatNullableString(evt.EndpointId) + ", " +
                _Driver.FormatNullableString(evt.EndpointName) + ", " +
                _Driver.FormatNullableString(evt.EndpointType) + ", " +
                _Driver.FormatNullableString(evt.Provider) + ", " +
                _Driver.FormatNullableString(evt.ApiFormat) + ", " +
                _Driver.FormatNullableString(evt.Model) + ", " +
                ...
                FormatNullableInt(evt.HttpStatusCode) + ", " +
                _Driver.FormatNullableString(evt.ErrorType) + ", " +
                _Driver.FormatNullableString(evt.ErrorMessage) + ", " +
                FormatNullableInt(evt.InputTokens) + ", " +
                FormatNullableInt(evt.OutputTokens) + ", " +
                FormatNullableInt(evt.TotalTokens) + ", " +
                FormatNullableInt(evt.ChunksInput) + ", " +
                FormatNullableInt(evt.ChunksOutput) + ", " +
                FormatNullableInt(evt.RetrievalQueryCount) + ", " +
                FormatNullableDouble(evt.EndpointLimiterWaitMs) + ", " +
                FormatNullableDouble(evt.RequestToHeadersMs) + ", " +
                FormatNullableDouble(evt.HeadersToFirstTokenMs) + ", " +
                FormatNullableDouble(evt.FirstTokenToLastTokenMs) + ", " +
                FormatNullableDouble(evt.ClientTotalMs) + ", " +
                FormatNullableDouble(evt.ProviderQueueMs) + ", " +
                FormatNullableDouble(evt.ProviderLoadMs) + ", " +
                FormatNullableDouble(evt.ProviderPromptEvalMs) + ", " +
                FormatNullableDouble(evt.ProviderGenerationMs) + ", " +
                FormatNullableDouble(evt.ProviderTotalMs) + ", " +
                FormatNullableDouble(evt.ProviderTokensPerSecond) + ", " +
                _Driver.FormatNullableString(evt.ProviderRequestId) + ", " +
                _Driver.FormatNullableString(evt.MetadataJson) + ", " +
                _Driver.FormatNullableString(evt.ProviderMetricsJson) + ", " +
                _Driver.FormatNullableString(evt.ProviderRawJson) + ", " +
